...
namespace MFST
{
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		case NS_NORULE: {        MFST_TRACE4(log, "------>NS_NORULE")
			* log.stream << "-------------------------------------------------------------------------------------" << std::endl;
			char* result = getDiagnosis(0, buf);
			*log.stream << result << std::endl; cout << result << std::endl;
			result = getDiagnosis(1, buf);
			*log.stream << result << std::endl; cout << result << std::endl;
			result = getDiagnosis(2, buf);
			*log.stream << result << std::endl; cout << result << std::endl;
			break;
			}
	...
	char* Mfst::getDiagnosis(short n, char* buf)
	{
		...
		if (n < MFST_DIAGN_NUMBER && (lpos = diagnosis[n].lenta_position) >= 0)
		{
			...
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "Ошибка %d: строка %d позиция %d. Синтаксическая ошибка %s; ", err.id, lex.lextable.table[lpos].sn, lex.lextable.table[lpos].pn, err.message);
			rc = buf;
		};
		return rc;
	};
